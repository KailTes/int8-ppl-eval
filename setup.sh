...
#   bash setup.sh quantize      /models/YourModel   — W8A8 量化 (llmcompressor, CPU)
#   bash setup.sh quantize_rtn  /models/YourModel   — W8A8 量化 (RTN safetensors, CPU, 无需额外依赖)
...
#   bash setup.sh eval_w8a8 /models/YourModel       — serve + eval (W8A8, llmcompressor)
#   bash setup.sh eval_rtn  /models/YourModel       — serve + eval (W8A8, RTN)
...
resolve_paths() {
    ...
    QUANT_OUTPUT="${out_base}/${MODEL_NAME}-W8A8"
    ...
    RESULTS_RTN="${RESULTS_DIR}/${MODEL_NAME}-rtn-w8a8"
}
...
# ---- quantize_rtn (纯 safetensors RTN 量化, 无需 llmcompressor) ----
do_quantize_rtn() {
    [ -z "${1:-}" ] && error "Usage: setup.sh quantize_rtn /path/to/model"
    resolve_paths "$1"
    info "=== RTN W8A8 Quantize (CPU, pure safetensors) ==="
    info "Input:  ${MODEL_PATH}"
    info "Output: ${QUANT_OUTPUT_RTN}"

    if [ -d "${QUANT_OUTPUT_RTN}" ] && [ -n "$(ls "${QUANT_OUTPUT_RTN}"/*.safetensors 2>/dev/null)" ]; then
        warn "Already exists: ${QUANT_OUTPUT_RTN}, skipping"
        return 0
    fi

    TORCH_DEVICE_BACKEND_AUTOLOAD=0 \
    python3 "${TASK_DIR}/quantize_safetensors_int8.py" \
        --model "${MODEL_PATH}" --output "${QUANT_OUTPUT_RTN}"
    info "Quantize done → ${QUANT_OUTPUT_RTN}"
}
...
# ---- eval_rtn: serve RTN model → eval ----
do_eval_rtn() {
    [ -z "${1:-}" ] && error "Usage: setup.sh eval_rtn /path/to/model"
    resolve_paths "$1"
    [ -d "${QUANT_OUTPUT_RTN}" ] || error "RTN quantized model not found: ${QUANT_OUTPUT_RTN}. Run 'setup.sh quantize_rtn' first."
    info "====== RTN W8A8 Pipeline: serve → eval ======"
    do_serve "${QUANT_OUTPUT_RTN}"
    do_eval "${QUANT_OUTPUT_RTN}" "${RESULTS_RTN}" || true
}
...
case "${1:-help}" in
    ...
    quantize_rtn) do_quantize_rtn "${2:-}" ;;
    serve)        do_serve "${2:-}" ;;
    eval)         do_eval "${2:-}" "${3:-}" ;;
    stop)         do_stop ;;
    eval_fp16)    do_eval_fp16 "${2:-}" ;;
    eval_w8a8)    do_eval_w8a8 "${2:-}" ;;
    eval_rtn)     do_eval_rtn "${2:-}" ;;
    all)
    ...
    *)
        ...
        echo "  quantize      /path/to/model   W8A8 quantization via llmcompressor (CPU)"
        echo "  quantize_rtn  /path/to/model   W8A8 quantization via RTN safetensors (CPU, no deps)"
        ...
        echo "  eval_w8a8     /path/to/model   W8A8 (llmcompressor): serve + eval"
        echo "  eval_rtn      /path/to/model   W8A8 (RTN): serve + eval"
        echo "  all           /path/to/model   Run all steps (llmcompressor)"
        echo ""
